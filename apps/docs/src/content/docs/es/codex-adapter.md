...
- Usa **`--force`** cuando el archivo es generado completamente por Kaddo — regenera toda la
  proyección.
- Usa **`--inject`** cuando el archivo pertenece al equipo — agrega o actualiza solo el bloque Kaddo.

Si corres `--inject` sobre un archivo **ya generado completamente por Kaddo** (tiene el header
`Generated by kaddo adapters install …` y no tiene markers), Kaddo no hace nada y te indica usar
`--force`, para evitar una segunda copia duplicada de la guía.
