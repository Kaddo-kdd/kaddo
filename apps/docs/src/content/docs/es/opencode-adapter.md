---
title: Adaptador OpenCode (AGENTS.md)
description: Genera un AGENTS.md para que OpenCode entienda cómo trabajar dentro de un repositorio gestionado con Kaddo — instrucciones nativas, sin pegar contexto a mano.
---

`kaddo adapters install opencode` genera un **`AGENTS.md`** en la raíz del proyecto para que
[OpenCode](https://opencode.ai) tenga instrucciones nativas para trabajar en un repo Kaddo — sin que
pegues el context pack, los prompts o las reglas en el chat.

```bash
kaddo adapters install opencode            # escribe AGENTS.md
kaddo export opencode                      # alias
kaddo adapters install opencode --dry-run  # preview, no escribe nada
kaddo adapters install opencode --inject   # agrega/actualiza solo el bloque Kaddo, preserva el resto
kaddo adapters install opencode --force    # sobrescribe un AGENTS.md existente
```
...
OpenCode lee instrucciones de agentes a nivel de repositorio desde `AGENTS.md` — el mismo archivo que
genera el [adapter de Codex](codex-adapter/). Este adapter sigue el mismo
[Adapter Contract](custom-adapters/) y reutiliza el mismo common core (metadata del proyecto, rutas
de conocimiento/derivadas, agentes, skills, detección de package manager, command fallback, merge
seguro, inject guard, markers neutros); solo cambia la etiqueta del target.
...
Después de `kaddo adapters install opencode --force`, valida que OpenCode realmente usa `AGENTS.md`:
...
Esta versión genera solo el `AGENTS.md` raíz. **No** genera comandos nativos de OpenCode (potencial
futuro `VS-067.1 — OpenCode Commands Projection`) ni agentes nativos de OpenCode (potencial futuro
`VS-067.2 — OpenCode Agents Projection`). Los agentes de Kaddo siguen en `knowledge/agents/` y las
skills en `knowledge/skills/`; la proyección solo le indica a OpenCode leer las relevantes.
