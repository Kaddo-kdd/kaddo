...
- **`kaddo module --init`** writes `architecture/module.yml` — a repo describing
...
- **`kaddo modules map`** writes `.kaddo/modules.yml` in the **architecture repo** —
  ...
  structure under `architecture/modules/<id>/`.
...
- **Global** (whole system): `kaddo add standards|security|stack|git-strategy`
  writes `architecture/<topic>.md` once for the system. See
  [Standards, security & stack](/modules/global-docs/) and
  [Git strategy](/modules/git-strategy/).
- **Module-level** (per repo): `architecture/modules/<id>/*.md`, generated by
  `kaddo modules map`.
