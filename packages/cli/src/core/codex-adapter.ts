...
import { exists, readDir, isDir, join, readFile } from '../utils/fs.js'
...
// Target-neutral markers (VS-066-followup). The block holds tool-agnostic Kaddo guidance, so the
// same markers work in AGENTS.md and CLAUDE.md. The legacy Codex-specific markers are still
// recognized and auto-migrated to the neutral form on the next inject/update.
export const KADDO_BEGIN_MARKER = '<!-- BEGIN KADDO ADAPTER -->'
export const KADDO_END_MARKER = '<!-- END KADDO ADAPTER -->'
const LEGACY_BEGIN_MARKER = '<!-- BEGIN KADDO CODEX ADAPTER -->'
const LEGACY_END_MARKER = '<!-- END KADDO CODEX ADAPTER -->'
const GENERATED_HEADER = 'Generated by `kaddo adapters install '

/** First index of either the neutral or legacy marker (whichever appears), or -1. */
function findMarker(content: string, neutral: string, legacy: string): number {
...
function markerLength(content: string, neutral: string, legacy: string): number {
...
/** Classify an existing adapter file so the command can pick a safe action. */
...
export function injectKaddoBlock(existing: string, ctx: CodexAdapterContext): MergeResult {
  ...
  // Recognize either the neutral or the legacy Codex marker; updates rewrite with neutral markers.
...
// ─────────────────────────────────────────────────────────────────────────────
// Adapter catalog, discovery & status (VS-070)
// ─────────────────────────────────────────────────────────────────────────────

export type AdapterInfo = { id: AdapterTarget; label: string; file: string; supportsInject: boolean }

/** Single source of truth for the supported adapters (catalog order is the display order). */
...
export function findAdapter(id: string): AdapterInfo | undefined {
...
export type AdapterStatus = AdapterInfo & {
  state: AdapterState
  /** Adapter id parsed from the generated-by header, when the file is fully generated. */
  originAdapter: AdapterTarget | null
  recommendedAction: string
}

/** Extract the adapter id from a `Generated by \`kaddo adapters install <id>\`` header, if present. */
export function parseOriginAdapter(content: string): AdapterTarget | null {
  const m = content.match(/Generated by `kaddo adapters install (\w+)`/)
  const id = m?.[1]
  return id && findAdapter(id) ? (id as AdapterTarget) : null
}

/** Refine an adapter file's content into a VS-070 status state (distinguishes legacy markers). */
export function classifyAdapterState(content: string | null): AdapterState {
...
function recommendedAction(a: AdapterInfo, state: AdapterState): string {
...
/**
 * Inspect every adapter's target file in `dir` and report its state. Read-only — never writes,
 * never runs git or an LLM. Files shared by several adapters (AGENTS.md) are read once.
 */
export function buildAdapterStatuses(dir: string): AdapterStatus[] {
...
/** Group statuses by target file so callers can surface AGENTS.md shared across adapters. */
export function buildSharedFileStatuses(statuses: AdapterStatus[]): SharedFileStatus[] {
  const byFile = new Map<string, AdapterStatus[]>()
