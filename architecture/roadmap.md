...
| VS-036 | `kaddo start` (work-item branch) | `kaddo start [id]` creates/switches to the work-item branch per the Git strategy so work never lands on `main` by accident; branch creation is the only Git action — Kaddo never commits, pushes or merges |
