...
| VS-019 | Demo Examples & Sample Repositories | ✅ Done | `examples/` demo repos for the four project states: new-project (Task Pilot), pre-ai-project (Loyalty Lite, incl. Guard drift demo), legacy-project (Old Orders), multirepo-workspace (Commerce Stack). Each has README.md + expected-flow.md + committed `.kaddo/`/`architecture/`/`sample/` artifacts; agent outputs clearly marked illustrative under `sample-agent-outputs/` and **aligned to the VS-018 templates** (including `## Quality checklist` + traceability front matter on module-design). Multirepo example ships a real `.kaddo/modules.yml` + per-module `architecture/modules/<id>/module-design.md`. New Examples docs page EN/ES + sidebar entry, README Examples section, linked from use-case pages. Also: docs "Writing a custom agent" guide (EN/ES) documenting the 9 required agent sections + 4 alignment rules, and docs body paragraphs justified (lists stay left-aligned). No CLI changes, no LLM, no demo apps. `examples/` excluded from pnpm workspace tooling |
| VS-018 | Complete Templates & Documentation Alignment | ✅ Done | Central typed template registry (`src/templates/registry.ts`) with 23 templates across core/architecture/module/operations/legacy; each carries purpose, when-to-use, output path, related command/agent, front matter where useful and a quality checklist. Agent prompts aligned to template output paths (tested). New Templates docs group (Overview + 5 category pages) EN/ES + README section. Minimum Sufficient Knowledge — guides, not forms. No new commands, no LLM |
| VS-017 | Multirepo Modules, Standards & Operational Agents | ✅ Done | `kaddo modules map/list` registers secondary repos in `.kaddo/modules.yml` and generates `architecture/modules/<id>/{module-design,stack,security,standards}.md` + `diagrams/` + `adrs/` (upsert by id, never overwrites). Global artifacts via `kaddo add standards/security/stack/git-strategy` (git default = GitHub Flow + Conventional Commits + SemVer, customizable in `.kaddo/git.yml`). Six operational agents (work-item, git-strategy, security, standards, stack, module-design). Docs+README EN/ES. No repo scanning, no Git/GitHub API, no security scanning, deterministic — no LLM |
